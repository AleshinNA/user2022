--------------------------------------------------------
--  File created - вторник-сентября-20-2022   
--------------------------------------------------------
--------------------------------------------------------
...
is
  ...
  l_sql      clob;
begin
 l_sql:='select distinct p.id_product_ver, p.name_product, p.price 
         from  product  p 
         ...
         order by '||in_orderfield ||' ' || in_ordertype;
           dbms_output.put_line(l_sql);
  open refcur for l_sql;
  return refcur;
end  f_select_product;                         
